...
/*
    ...
    - Cada hilo ejecutara una columna de la matriz de resultado S 
...
__global__ void add_matrix(float *A, float *B,float *S, int n) {
    // Dado que aqui cada hilo ejecutara una columna el indice debe ir de 0 a n solamente 
    int columna = blockDim.x*blockIdx.x + threadIdx.x;
    if(columna < n){
        int i,id;
        // Debemos hallar la posicion en el arreglo que corresponden con ese numero de fila
        for(i=0;i<n;i++){
            id = n*i + columna;
            S[id] = A[id] + B[id];
        }
